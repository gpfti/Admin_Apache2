...
1. **Redirección condicional**
   ...
   * Crea un archivo de configuración en `/etc/apache2/sites-available/practica-containers.conf`.
   * Dentro del archivo, usa un contenedor `<IfDefine>` que, **si se arranca el servidor con el parámetro `CERRADO`**, redirija todas las peticiones hacia `https://www.google.com`.

2. **Bloqueo de archivo sensible**
...
6. **Comprobación por versión**

   * Añade un bloque `<IfVersion>` que solo se ejecute en versiones de Apache mayores o iguales a la 2.4.50. Puedes poner simplemente un comentario dentro del bloque indicando “versión válida”.
...
1. Habilita el nuevo archivo de configuración con `sudo a2ensite practica-containers.conf`.
2. Recarga Apache: `sudo systemctl reload apache2`.
3. Para probar el `<IfDefine>`, reinicia el servidor con:

```bash
sudo apachectl -D CERRADO
```

4. Usa herramientas como `curl` o el navegador para probar los diferentes comportamientos.
