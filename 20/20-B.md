...
## **¿Qué son las Directivas en Apache?**
...
## **¿Y qué son los Contenedores (Containers)?**
...
Este contenedor con directivas indica que para todo lo que esté en /var/www/html, permite listar el contenido del directorio, seguir enlaces simbólicos, y deja acceder a todo el mundo.
...
Patrón para entender estos comandos:
 - "a2en" Habilita + "lo que sea":  
 - "a2dis" Deshabilita + "lo que sea":  
...
Apache es un servidor modular. Esto quiere decir que solo se incluye la funcionalidad más básica en el servidor. Las características extendidas están disponibles a través de módulos que pueden ser cargados en Apache, es decir, que muchas de sus funcionalidades no vienen activadas por defecto, sino que se añaden en forma de módulos. 
...
##### 2. Usar directivas solo si un módulo está disponible: `<IfModule>`
...
Uso de Archivos `.htaccess` en Apache

Objetivo: Familiarizarse con el uso de archivos `.htaccess` en Apache para gestionar la configuración personalizada de un sitio web.

Instrucciones:

1. Investiga y comprende el propósito y la sintaxis de los archivos `.htaccess` en Apache. Experimenta con diferentes directivas (explicadas en el texto) para realizar tareas como redireccionamientos de URL, control de acceso, personalización de encabezados HTTP, compresión de archivos, etc. Realiza pruebas en tu sitio web para verificar que las directivas en el archivo .htaccess estén funcionando según lo esperado.

2. **Prepara tu entorno:**  
   - Crea un directorio llamado `practica_htaccess` dentro del directorio raíz de tu servidor web (por ejemplo: `/var/www/html/practica_htaccess`).

3. **Habilita el uso de `.htaccess`:**  
   - Asegúrate de que en la configuración de Apache esté permitido el uso de archivos `.htaccess` (específicamente, que la directiva `AllowOverride` esté en `All` para ese directorio).

4. **Crea un archivo `.htaccess`** dentro del directorio `practica_htaccess` y agrega las siguientes directivas específicas:

   a) **Redirigir** todas las visitas de `http://tuservidor/practica_htaccess/` a `http://tuservidor/practica_htaccess/nueva-pagina.html`.  

   b) **Restringir** el acceso a todo el directorio solo a tu dirección IP (por ejemplo, `123.45.67.89`).

   c) **Modificar encabezados HTTP** para que todas las respuestas incluyan el header `X-Ejemplo: PracticaHTACCESS`.

   d) **Habilitar compresión** para archivos `.html` y `.css` si no está habilitada en el servidor globalmente.

5. **Crea los archivos de prueba:**
   - Un archivo `index.html` con cualquier contenido.
   - Un archivo `nueva-pagina.html` con un mensaje como “Has sido redirigido correctamente.”

6. **Prueba cada funcionalidad:**
   - Visita el sitio y comprueba la redirección.
   - Intenta acceder desde otra IP para verificar la restricción.
   - Usa herramientas como `curl` o la pestaña "Network" de tu navegador para ver si el header personalizado está presente.
   - Verifica que los archivos `.html` y `.css` son servidos comprimidos (puedes usar herramientas como `curl -I --compressed`).
